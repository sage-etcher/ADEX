...
umount (int disk)
{
  if (g_nsd[disk - 1].fdd != NULL)
    {
      fclose (g_nsd[disk - 1].fdd);
    }
  g_nsd[disk - 1].fdd = NULL;
...
fdc_in (BYTE port_lo)
{
  ...
  switch (port_lo)
    ...
    case 1:
      ...
      store_sector_buffer ();
      g_floppy->fd_bytes_to_xfer = 0x200;

      break;

    case 2:
      xlog (FDC, "82: CLEAR DISK READ FLAG  \n");
      g_floppy->fd_disk_read_flag = FALSE;
      break;
    case 3:
...
fdc_out (BYTE port_lo, BYTE data)
{
  ...
  switch (port_lo)
    ...
    case 3:
      ...
      g_floppy->fd_disk_write_flag = TRUE;
      g_floppy->fd_byte_ptr = 0;
      g_floppy->fdc_state = 200;
...
/*********************************************************************************/
  ...
  prev_track = g_floppy->fd_track_num;
...
write_sector_to_disk ()
{
  ...
  if (g_floppy->fd_side)
    ...
    {
      store_sect_num = (g_floppy->fd_track_num * 10) + g_floppy->fd_sector_num;
    }

  xlog (FDC, "write_sector_to_disk: store_sect_num = %d\n", store_sect_num);
  offset = (long) (store_sect_num * DSECTOR);
  fseek (g_floppy->fdd, offset, SEEK_SET);
  xlog (FDC, "Floppy = \"%s\"\n", g_floppy->fdfn);
  fwrite (&g_floppy->fd_databuffer[0], DSECTOR, 1, g_floppy->fdd);
  g_floppy->fd_byte_ptr = 0;
}
...
store_sector_buffer ()
{
  ...
  second_sync = (g_floppy->fd_sector_num + (16 * g_floppy->fd_track_num)) & 0x0ff;
  ...
  fseek (g_floppy->fdd, offset, SEEK_SET);
  xlog (FDC, "Floppy = \"%s\"\n", g_floppy->fdfn);
  fread (&g_floppy->fd_databuffer[1], DSECTOR, 1, g_floppy->fdd);
  g_floppy->fd_crc_val = 0;
  for (i = 1; i < 0x201; i++)
    {
      calc_crc (g_floppy->fd_databuffer[i]);
    }
  g_floppy->fd_databuffer[0x201] = g_floppy->fd_crc_val;
...
increment_sector_num ()
{
  ...
  g_floppy->fdc_state_sector_num = g_floppy->fd_sector_num;
  if (g_floppy->fd_sector_num == 9)
    {
...
floppy_state ()
{
  ...
  if (g_display_flag_counter)
    {
      g_display_flag_counter--;
    }
  if (!g_display_flag_counter)
    {
      g_display_flag_counter = 200;
      g_display_flag = TRUE;
    }
...
/***************  SECTOR MARK HIGH  **************/
    ...
    case 15:
      ...
      g_floppy->fdc_state = 18;
      g_floppy->fdc_state_counter = 60;
      g_floppy_pulse_flag = TRUE;
      g_floppy_controller_clock = 0;
      break;
    case 18:
